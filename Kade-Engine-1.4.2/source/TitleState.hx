...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		titleBG.setGraphicSize(Std.int(titleBG.width * 4));
		...
		gfDance.animation.addByPrefix('TitleGFAnimation', 'GF FEAR ', 24);
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			dontAnimateGF = pressedEnter;
			gfDance.animation.play("TitleGFAnimation");

			FlxG.camera.flash(FlxColor.WHITE, 1);
			FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);

			transitioning = true; // yo i wish i was too
			...
			new FlxTimer().start(2, function(tmr:FlxTimer)
				{
					...
					http.onData = function (data:String)
					{
						...
						// Got rid of the outdated thing again, lul
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(titleBG, {"scale.x": 1, "scale.y": 1}, 4, {
				ease: FlxEase.expoOut
			});
		}
	}
